...
- **Client identity = injected wallet.** In the Client view the buyer is the connected wallet (`buyer_id` = lowercase address, `lib/injected-wallet.tsx`, `useActiveAccount()` in `lib/identity.ts`); it signs approve/buy and one `personal_sign` of `PASS_KEY_MESSAGE` derives the pass encryption key (`keyFromSignature`, packages/payments). Swarm ID stays the Provider identity and is optional for clients: when signed in at purchase its grantee key is attached to the sale so private files can be granted; decryption always runs in Swarm ID. The marketplace and service pages stay public; My passes and buying need the wallet.
- **Role:** client/provider is a localStorage preference keyed by Swarm identity id, never access control. Guests have a separate preference. The header switch changes workspace without reconnecting Swarm ID. Both sections remain reachable by explicitly switching views; AppShell hides the other workspace's pages. See UI guidance for navigation rules.
