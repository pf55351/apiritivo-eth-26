...
export function EnsBadge({ service }: { service: ArkivService }) {
  ...
  const tone = complete ? "border-success/40 text-success" : ok ? "border-accent-text/40 text-accent-text" : "border-line text-subtle";
...
function Row({ label, ok, value }: { label: string; ok: boolean | null; value: string }) {
  ...
  const tone = ok === null ? "text-subtle" : ok ? "text-success" : "text-subtle";
  return (
    <div className="flex items-start gap-2 text-xs">
      <span className={`font-mono ${tone}`}>{icon}</span>
      <span className="text-muted">{label}</span>
      <span className="ml-auto max-w-[55%] truncate font-mono text-content" title={value}>
        {value}
      </span>
    </div>
  );
  ...
  return (
    <section className="rounded-panel bg-surface p-6">
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
        <a href={ensAppUrl(ens.name)} target="_blank" rel="noreferrer" className="break-all font-mono text-base text-content hover:text-accent-text">
          {ens.name} ↗
        </a>
        {v ? (
          <span
            className={`rounded-full border px-2 py-0.5 text-[11px] ${v.complete ? "border-success/40 text-success" : v.addressOk ? "border-accent-text/40 text-accent-text" : "border-danger/40 text-danger"}`}
          >
      ...
      {ens.status === "error" ? <p className="mt-3 text-xs text-danger">ENS resolution failed: {ens.error}</p> : null}

      {isProviderView && missing && recipe.length > 0 ? (
        <div className="mt-4 rounded-2xl border border-line bg-surface/40 p-3">
          <p className="text-xs font-semibold text-content">Records to set in the ENS app</p>
          <p className="mt-1 text-[11px] text-subtle">Open the name, edit records, paste these three. Reads update within a minute.</p>
          <div className="mt-2 space-y-1.5">
            {recipe.map((rec) => (
              <div key={rec.kind} className="flex items-center gap-2 text-[11px]">
                <span className="w-28 shrink-0 text-subtle">{rec.key}</span>
                <code className="min-w-0 flex-1 truncate font-mono text-content" title={rec.value}>
                  {rec.value}
                </code>
                <button
                  type="button"
