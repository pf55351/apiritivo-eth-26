...
import { formatAccessDuration, formatPriceUsdc, type ArkivService } from "@apiritivo/shared";
...
export function ServiceCard({ service, showAvailability = false }: { service: ArkivService; showAvailability?: boolean }) {
  ...
  return (
    <article className="card card-interactive group relative flex min-w-0 flex-col gap-5 p-5 animate-fade-up">
      ...
      <div className="mt-auto flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1 border-t border-line pt-4">
        <span className="text-xl font-semibold tracking-tight text-content">{service.priceUsdc ? formatPriceUsdc(service.priceUsdc) : "Free"}</span>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <span className="relative z-10 flex gap-2">
          <ProofChip label="Arkiv" title="Open entity in the Arkiv Data Explorer (Tiramisu)" href={service.entityKey ? arkivEntityUrl(service.entityKey) : undefined} />
          ...
          {service.privateAttachment ? (
            <span className="inline-flex items-center gap-1 rounded-md border border-olive-400/30 bg-olive-400/10 px-2 py-0.5 font-mono text-[11px] text-olive-400" title={`Private file: ${service.privateAttachment.name} (Swarm ACT)`}>
              🔒 private file
            </span>
          ) : null}
        </span>
      ...
      <span className="inline-flex items-center justify-between border-t border-line pt-4 text-xs font-semibold text-muted transition-colors group-hover:text-accent-text">
        Inspect service <span aria-hidden="true">↗</span>
      </span>
    </article>
  );
}
