...
describe("access pass + sale parsing", () => {
  ...
  test("reads the ownership fields: secret_hash attribute and encrypted secret in the payload", () => {
    const pass = parseAccessPassEntity({
      ...
      expiresAt: 999n,
      attributes: attrs({ entity_type: "access_pass", buyer_id: "b", tx_hash: "0x" + "2".repeat(64), paid_usdc: { type: "dec", value: "0.5" }, chain_id: 43113, secret_hash: "0x" + "c".repeat(64) }),
  ...
  test("carries the buyer's Swarm public key for ACT grants", () => {
    ...
    const pass = parseAccessPassEntity({ key: key as `0x${string}`, expiresAt: 9n, attributes: attrs({ ...Object.fromEntries(Object.entries(passAttrs).map(([k, v]) => [k, v.value])), buyer_pubkey: pk }), toJson: () => ({}) });
    expect(pass?.buyerPublicKey).toBe(pk);
    const sale = parseSaleEntity({ key: key as `0x${string}`, attributes: attrs({ entity_type: "sale", buyer_id: "b", tx_hash: "0x" + "3".repeat(64), paid_usdc: { type: "dec", value: "1" }, chain_id: 43113, buyer_pubkey: pk }), toJson: () => ({}) });
...
describe("private files (Swarm ACT)", () => {
  ...
  test("a service with a private file exposes name, size, type and ACT refs", () => {
    const svc = parseServiceEntity({
      key: key as `0x${string}`,
      attributes: attrs({ private_name: "docs.md", private_bytes: { type: "u64", value: 1234n }, private_type: "text/markdown", private_enc_ref: enc, private_history_ref: hist, private_pubkey: pub }),
  ...
  test("a broken private ref drops the whole service (never a half attachment)", () => {
    const svc = parseServiceEntity({ key: key as `0x${string}`, attributes: attrs({ private_enc_ref: "zzz", private_history_ref: hist, private_pubkey: pub }), toJson: () => ({ name: "Market", description: "d" }) });
    expect(svc).toBeNull();
  });
  test("parses a grant and rejects other entity types", () => {
    ...
    expect(grant).toMatchObject({ grantKey: key, serviceId: "market-data-a81f", providerId: "swarm-1", buyerId: "buyer-1", buyerPublicKey: pub, historyRef: hist, encryptedRef: enc, publisherPubKey: pub, createdAtBlock: "42" });
