...
| --- | --- | --- |
| Buyer | Swarm-derived EVM account, or injected wallet such as MetaMask; signs token approval and purchase | Browser, `packages/payments/src/browser.ts` |
...
Wallet derivation uses `deriveAppSecret("apiperitivo:wallet:v1")`; pass encryption uses the separate `"apiritivo:pass-crypt:v1"` label. The identity and app origin affect derivation. The pre-rename wallet label is deliberate. A local role selection is a UI preference, not authorization.
